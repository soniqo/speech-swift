...
- **Qwen3-Chat** — Chat com LLM no dispositivo (0.6B, CoreML/Neural Engine, INT4/INT8, tokens em streaming, modo de raciocinio)
...
| Kokoro-82M | Texto → Fala | Nao | 10 idiomas | [CoreML FP16](https://huggingface.co/aufklarer/Kokoro-82M-CoreML) ~325 MB · [CoreML INT8](https://huggingface.co/aufklarer/Kokoro-82M-CoreML-INT8) 89 MB |
| Qwen3-0.6B Chat | Texto → Texto (LLM) | Sim (streaming) | Multi | [CoreML INT4](https://huggingface.co/aufklarer/Qwen3-0.6B-Chat-CoreML) 318 MB · [CoreML INT8](https://huggingface.co/aufklarer/Qwen3-0.6B-Chat-CoreML) 571 MB |
...
- **Kokoro TTS**: TTS leve pronto para iOS (82M parametros), CoreML/Neural Engine, 50 vozes, 10 idiomas, nao-autorregressivo (passagem unica)
...
Nao-autorregressivo — passagem unica, sem loop de amostragem. Veja [Forced Aligner](docs/inference/forced-aligner.md) para detalhes da arquitetura.
...
// Baixa ~325 MB na primeira execucao (modelos CoreML + embeddings de voz + dicionarios)
...
50 vozes predefinidas em 10 idiomas. Nao-autorregressivo — passagem unica no CoreML, sem loop de amostragem. Executa no Neural Engine, liberando a GPU completamente.
...
| Kokoro-82M | CoreML (Neural Engine) | ~45ms | ~45ms | ~45ms | N/A (nao-autorregressivo) |
| Apple `AVSpeechSynthesizer` | AVFoundation | 0.08s | 0.08s | 0.17s (RTF 0.02) | N/A |

> Qwen3-TTS gera fala natural e expressiva com prosodia e emocao, executando **mais rapido que tempo real** (RTF < 1.0). A sintese em streaming entrega o primeiro bloco de audio em ~120ms. Kokoro-82M executa inteiramente no Neural Engine com uma unica passagem — ~45ms independente da duracao da saida, ideal para iOS. O TTS integrado da Apple e mais rapido, mas produz fala robotica e monotona.

### PersonaPlex (Fala-a-Fala)

| Modelo | Framework | ms/passo | RTF | Observacoes |
|--------|-----------|----------|-----|-------------|
| PersonaPlex-7B (4-bit) | MLX Swift (release) | ~68ms | ~0.87 | Entrada 20s → saida 36s em ~31s |

> PersonaPlex executa a ~68ms/passo — bem abaixo do limiar de tempo real de 80ms a 12.5 Hz, alcancando inferencia **mais rapida que tempo real** (RTF < 1.0). Tanto o transformer temporal quanto o depformer sao quantizados em 4-bit.
...
**Inferencia:** [ASR Inference](docs/inference/asr-inference.md), [TTS Inference](docs/inference/qwen3-tts-inference.md), [Forced Aligner](docs/inference/forced-aligner.md), [FireRedVAD](docs/inference/fireredvad.md), [Silero VAD](docs/inference/silero-vad.md), [Speaker Diarization](docs/inference/speaker-diarization.md), [Speech Enhancement](docs/inference/speech-enhancement.md)
...
| **Streaming** | Sim (~120ms primeiro bloco) | Nao (passagem unica, ~45ms) | Nao | Sim |
| **Clonagem de voz** | Sim | Nao | Nao | Sim |
| **Vozes** | 9 integradas + clonar qualquer | 50 vozes predefinidas | ~50 vozes do sistema | 1000+ |
| **Idiomas** | 10 | 10 | 60+ | 30+ |
...
Kokoro TTS, Qwen3-Chat, Silero VAD, Parakeet ASR, DeepFilterNet3 e WeSpeaker funcionam no iOS 17+ via CoreML no Neural Engine. Modelos baseados em MLX (Qwen3-ASR, Qwen3-TTS, PersonaPlex) requerem macOS 14+ em Apple Silicon.
...
De ~3 MB (Silero VAD) ate ~6.5 GB (PersonaPlex 7B). Kokoro TTS usa ~200 MB, Qwen3-ASR ~2.2 GB. Veja a tabela de [Requisitos de Memoria](#requisitos-de-memoria) para detalhes completos.
