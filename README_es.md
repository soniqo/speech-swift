...
## Proyectos creados con Speech Swift

16 repositorios públicos con referencias verificables al paquete Speech Swift.
...
- **[CSM (Conversational Speech Model)](docs/models/csm.md)** — TTS conversacional con clonación de voz zero-shot a partir de un clip de referencia + transcripción (Sesame CSM-1B, backbone Llama-1B + códec Mimi, MLX int8/fp16, 24 kHz, inglés; Apache-2.0)
...
- **[LocalVQE v1.4-AEC](https://soniqo.audio/es/guides/echo-cancellation)** — Cancelación de eco acústico en streaming con flujos separados y sincronizados de micrófono y referencia de reproducción (Core ML + filtro adaptativo nativo, 16 kHz, 16 ms de latencia algorítmica)
...
- **[Smart Turn](docs/models/smart-turn-v3.md)** — Detección de fin de turno a partir del audio (Pipecat Smart Turn v3.2, CoreML, 8M, 23 idiomas; confirma las pausas de Silero VAD para que una pausa a mitad de frase no termine el turno)
...
- **10 sep 2026** — [NVIDIA Nemotron VoiceChat en Apple Silicon — Voz full-duplex con herramientas](https://blog.ivan.digital/nvidia-nemotron-voicechat-on-apple-silicon-full-duplex-speech-with-tools-6165625d55c4)
...
| [CSM](docs/models/csm.md) | Texto → Voz (conversacional, clonación zero-shot) | MLX | 1B (int8 / fp16) | EN |
...
| [VoiceChat 11B](docs/models/voicechat.md) | Voz → Voz + Texto | MLX | 11B (INT5 / INT8) | EN |
...
| [Smart Turn v3.2](https://huggingface.co/aufklarer/Smart-Turn-v3.2-CoreML) | Detección de fin de turno | CoreML | 8M | 23 idiomas |
...
| [Nemotron 3](docs/models/nemotron3-diarization.md) | Diarización (hasta 8 hablantes) | CoreML / MLX (INT8) | 100M | Agnóstico |
| [DeepFilterNet3](https://soniqo.audio/es/guides/denoise) | Mejora de voz | CoreML | 2.1M | Agnóstico |
| [LocalVQE v1.4-AEC](https://soniqo.audio/es/guides/echo-cancellation) | Cancelación de eco acústico | CoreML + C++ | 200K + 2,742 | Agnóstico |
...
- **[Soniqo VoiceChat CLI](docs/inference/voicechat.md)** ([guía](https://soniqo.audio/es/guides/speech-to-speech)) — Asistente de terminal Nemotron 11B dúplex nativo con subtítulos RNN-T en vivo, control de turnos del modelo, detalle EAR-TTS adaptativo y herramientas MCP opcionales. La configuración incluida de Apple Reminders solo expone crear, listar y actualizar.
- **[SpeechDemo](Examples/SpeechDemo/)** — Dictado y síntesis TTS en una interfaz de pestañas. macOS.

Ejecuta la demo de recordatorios de VoiceChat después de compilar en release:
...
Añade `--debug-timeline` para ver marcas de tiempo de frases, fin de pronunciación generada y ciclo de vida de herramientas decodificado por el modelo. Puede revelar argumentos de herramientas; no lo uses en registros compartidos. Consulta el README de cada app o la guía enlazada para los detalles de compilación y ejecución.
...
### Cancelación de eco acústico — [guía completa →](https://soniqo.audio/es/guides/echo-cancellation)
