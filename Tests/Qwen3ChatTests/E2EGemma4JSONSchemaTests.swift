...
final class E2EGemma4JSONSchemaTests: XCTestCase {
    ...
    private func run(_ chat: Gemma4Chat, _ sampling: ChatSamplingConfig) throws -> Run {
        let constraint = try chat.makeConstraint(sampling.responseFormat)
    ...
    private func assertValidFactDocument(_ text: String, file: StaticString = #filePath, line: UInt = #line) throws {
        ...
        let states = matcher.advance(matcher.initial, bytes: Array(text.utf8))
        XCTAssertTrue(matcher.isDone(states), "reply must be one complete schema-valid document", file: file, line: line)
