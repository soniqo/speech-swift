...
- **`speech voice-chat`**: Uses `FullDuplexAudioIO`, which owns one
  `AVAudioEngine` for both microphone capture and an `AVAudioPlayerNode`.
  Generated 80 ms VoiceChat frames are scheduled in advance, with a default
  three-frame startup cushion. If the scheduled queue drains, the next restart
  waits for an adaptive recovery cushion (eight frames for the default
  configuration) instead of repeatedly restarting on three frames. The first
  response therefore keeps its 240 ms playback cost, while a transient slow
  inference frame is less likely to turn into repeated audible cuts. The
  dashboard calls these drains `speaker gaps`. A recovery buffer absorbs
  transient jitter; it cannot correct sustained model compute above 80 ms per
  80 ms audio frame, so the VoiceChat command separately reduces refinement
  work and bounds stale microphone latency in that case. The
  microphone is resampled to 16 kHz. Model-native
  22.05 kHz output is resampled to the Voice Processing client rate before it
  reaches the player node, because AEC requires matching client-side capture
  and playback sample rates. On macOS, capture starts before the player is
  attached: Voice Processing negotiates its output format asynchronously, so
  the live player format is read only after the capture graph is running.
  Microphone speech captured while a tool owns VoiceChat's shared language
  cache is replayed causally after the result. Those replay events keep model
  state aligned but set `playbackRequired` to false while the assistant is
  idle, preventing already-elapsed provider-wait silence from being scheduled
  a second time.
  Live codec rendering decodes a bounded eight-frame window and emits its final
  80 ms frame; saved full-context audio is decoded continuously. The
  `voicechat-bench` signal report compares both paths. A pause present in both
  renders was generated by EAR-TTS or the codec rather than by the scheduled
  player, while a discontinuity present only in the live render points to the
  moving codec window or playback stitching. Speaker gaps remain a separate
  queue-underflow metric. Onset-pop analysis separately scans the generated
  lead-in and first 50 ms of sustained speech, so an isolated codec-edge click
  no longer disappears behind the sustained-audibility latency threshold.
...
try audio.start { microphoneSamples in
    inputQueue.append(microphoneSamples)
}
audio.schedulePlayback(modelFrame)
```

`FullDuplexAudioIO` is deliberately separate from `StreamingAudioPlayer`.
The latter's pull-based source node remains the gap-resistant choice for
standalone TTS, while the scheduled player is the AEC-compatible choice when
capture and playback are simultaneous.
