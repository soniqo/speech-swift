...
On-device text generation backends exposed by the `Qwen3Chat` target. The module now covers:
...
The shared `PreQuantizedEmbedding` path supports non-4/8-bit packing, so 5-bit MLX exports can be used directly.
...
// Dense Qwen3 MLX (for larger on-device chat)
