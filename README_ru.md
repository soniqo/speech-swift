...
Release-бенчмарк VoiceChat (M5 Pro, 48 ГБ; входные live-блоки по 80 мс; контролируемый тест на 120 кадров):
...
Значения первого токена/аудио — это прогретая вычислительная задержка первого кадра устного ответа, а не выученная задержка смены реплики. Оба варианта пока медленнее реального времени.
